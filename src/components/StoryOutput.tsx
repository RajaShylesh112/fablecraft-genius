...
const StoryOutput: React.FC<StoryOutputProps> = ({
  ...
  return (
    <GlowEffect 
      position="center" 
      color="hsla(210, 100%, 50%, 0.2)" 
      size="xl" 
      intensity="low"
      containerClassName={cn(className)}
    >
      <div className={cn('relative rounded-2xl overflow-hidden glass-panel', 'glow-container')}>
        ...
        >
          {title && (
            <h3 className="text-xl md:text-2xl font-medium mb-4 font-display text-glow-sm">
              <AnimatedText 
                text={title}
                reveal={{ 
                  type: "words",
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none">
            {showContent && !isLoading ? (
              content.split('\n').map((paragraph, index) => (
                paragraph ? (
                  <p key={index} className="my-3 text-foreground/90 font-sans">
                    <AnimatedText 
                      text={paragraph}
                      reveal={{ 
                        type: "words",
